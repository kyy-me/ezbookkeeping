...
function getAllMeridiemIndicatorNames(translateFn) {
    ...
    for (
        let i = 0;
        i < datetimeConstants.allMeridiemIndicatorsArray.length;
        i++
    ) {
        const indicatorName = datetimeConstants.allMeridiemIndicatorsArray[i];
        allMeridiemIndicatorNames.push(
            translateFn(`datetime.${indicatorName}.content`)
        );
    }
...
function getAmountPrependAndAppendText(
    ...
    const currencyName = getCurrencyName(currencyCode, translateFn);
    return getAmountPrependAndAppendCurrencySymbol(
        currencyDisplayType,
        currencyCode,
        currencyName
    );
...
function getAllTransactionEditScopeTypes(translateFn) {
    ...
    for (const typeName in transactionConstants.allTransactionEditScopeTypes) {
        if (
            !Object.prototype.hasOwnProperty.call(
                transactionConstants.allTransactionEditScopeTypes,
                typeName
            )
        ) {
            continue;
        ...
        const editScopeType =
            transactionConstants.allTransactionEditScopeTypes[typeName];
...
function setLanguage(i18nGlobal, locale, force) {
    ...
    dayjs.updateLocale(locale, {
        ...
        meridiem: function (hours) {
            if (isPM(hours)) {
                return i18nGlobal.t(
                    `datetime.${datetimeConstants.allMeridiemIndicators.PM}.content`
                );
            } else {
                return i18nGlobal.t(
                    `datetime.${datetimeConstants.allMeridiemIndicators.AM}.content`
                );
            }
        },
    });

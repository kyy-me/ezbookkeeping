    ...
    <f7-page with-subnavbar @page:afterin="onPageAfterIn">
        <f7-navbar>
            ...
            <f7-nav-right>
                ...
                ></f7-link>
                <f7-link
                    :class="{ disabled: inputIsEmpty || submitting }"
            ...
            <f7-subnavbar>
                <f7-segmented strong :class="{ readonly: mode !== 'add' }">
                    <f7-button
                        :text="$t('Expense')"
                        :active="
                            transaction.type === allTransactionTypes.Expense
                        "
                        :disabled="
                            mode !== 'add' &&
                            transaction.type !== allTransactionTypes.Expense
                        ...
                        v-if="
                            transaction.type !==
                            allTransactionTypes.ModifyBalance
                        "
                        @click="transaction.type = allTransactionTypes.Expense"
                    ></f7-button>
                    <f7-button
                        :text="$t('Income')"
                        :active="
                        ...
                        v-if="
                            ...
                            allTransactionTypes.ModifyBalance
                        "
                        @click="transaction.type = allTransactionTypes.Income"
                    ></f7-button>
                    <f7-button
                        :text="$t('Transfer')"
                        :active="
                            transaction.type === allTransactionTypes.Transfer
                        "
                        :disabled="
                            mode !== 'add' &&
                            transaction.type !== allTransactionTypes.Transfer
                        ...
                        v-if="
                            ...
                            allTransactionTypes.ModifyBalance
                        "
                        @click="transaction.type = allTransactionTypes.Transfer"
                    ></f7-button>
                    <f7-button
                        :text="$t('Modify Balance')"
                        :active="
                            transaction.type ===
                            allTransactionTypes.ModifyBalance
                        "
        ...
        >
            ...
            <f7-list-item
                ...
                header="Transaction Timezone"
                title="(UTC XX:XX) System Default"
                link="#"
            ...
            <f7-list-input
                type="textarea"
                label="Description"
                placeholder="Your transaction description (optional)"
            ></f7-list-input>
        </f7-list>
        ...
        >
            ...
            >
                <template #title>
                    <div
                        class="list-item-custom-title"
                        v-if="hasAvailableExpenseCategories"
                    >
                        <span>{{
                            getPrimaryCategoryName(
                                transaction.expenseCategory,
                                allCategories[allCategoryTypes.Expense]
                            )
                        }}</span>
                        <f7-icon
                            class="category-separate-icon"
                            f7="chevron_right"
                        ></f7-icon>
                        <span>{{
                    ...
                    <div
                        class="list-item-custom-title"
                        v-else-if="!hasAvailableExpenseCategories"
                    >
                        <span>{{ $t("None") }}</span>
                    </div>
                </template>
            ...
            >
                <template #title>
                    <div
                        class="list-item-custom-title"
                        v-if="hasAvailableIncomeCategories"
                    >
                        <span>{{
                            getPrimaryCategoryName(
                                transaction.incomeCategory,
                                allCategories[allCategoryTypes.Income]
                            )
                        }}</span>
                        <f7-icon
                            class="category-separate-icon"
                    ...
                    <div
                        class="list-item-custom-title"
                        v-else-if="!hasAvailableIncomeCategories"
                    >
                        <span>{{ $t("None") }}</span>
                    </div>
                </template>
            ...
            >
                <template #title>
                    <div
                        class="list-item-custom-title"
                        v-if="hasAvailableTransferCategories"
                    >
                        <span>{{
                            getPrimaryCategoryName(
                                transaction.transferCategory,
                                allCategories[allCategoryTypes.Transfer]
                            )
                        }}</span>
                        <f7-icon
                            class="category-separate-icon"
                    ...
                    <div
                        class="list-item-custom-title"
                        v-else-if="!hasAvailableTransferCategories"
                    >
                        <span>{{ $t("None") }}</span>
                    </div>
                </template>
            ...
            <f7-list-item
                ...
                :class="{
                    readonly:
                        mode === 'view' &&
                        transaction.utcOffset === currentTimezoneOffsetMinutes,
                }"
                :header="$t('Transaction Time')"
                :title="transactionDisplayTime"
                @click="
                    mode !== 'view'
                        ? (showTransactionDateTimeSheet = true)
                        : (showTimeInDefaultTimezone =
                              !showTimeInDefaultTimezone)
                "
            >
            ...
            >
                <select v-model="transaction.timeZone">
                    ...
                    >
                        {{ timezone.displayNameWithUtcOffset }}
                    </option>
                </select>
                <template #title>
                    <f7-block
                        class="list-item-custom-title no-padding no-margin"
                    >
                        <span>{{ `(${transactionDisplayTimezone})` }}</span>
                        <span
                            class="transaction-edit-timezone-name"
                            v-if="
                                transaction.timeZone ||
                                transaction.timeZone === ''
                            "
                            >{{ transactionDisplayTimezoneName }}</span
                        >
                        <span
                            class="transaction-edit-timezone-name"
                            v-else-if="
                                ...
                                transaction.timeZone !== ''
                            "
                            >{{ transactionTimezoneTimeDifference }}</span
                        >
                    </f7-block>
                </template>
            </f7-list-item>
            ...
            <f7-list-item
                ...
                class="list-item-with-header-and-title list-item-title-hide-overflow"
                :class="{
                    readonly: mode === 'view' && !transaction.geoLocation,
                }"
            ...
            >
                <template #title>
                    <f7-block
                        class="list-item-custom-title no-padding no-margin"
                    >
                        <span v-if="transaction.geoLocation">{{
                            `(${transaction.geoLocation.longitude}, ${transaction.geoLocation.latitude})`
                        }}</span>
                        <span v-else-if="!transaction.geoLocation">{{
            ...
            >
                ...
                <template #footer>
                    <f7-block
                        class="margin-top-half no-padding no-margin"
                        v-if="transaction.tagIds && transaction.tagIds.length"
                    >
                        <f7-chip
                            media-bg-color="black"
                            class="transaction-edit-tag"
                    ...
                    <f7-block
                        class="margin-top-half no-padding no-margin"
                        v-else-if="
                            !transaction.tagIds || !transaction.tagIds.length
                        "
                    >
                        <f7-chip
                            class="transaction-edit-tag"
            ...
            <f7-list-input
                ...
                :placeholder="
                    mode !== 'view'
                        ? $t('Your transaction description (optional)')
                        : ''
                "
        ...
        <f7-actions
            ...
            close-on-escape
            :opened="showGeoLocationActionSheet"
            @actions:closed="showGeoLocationActionSheet = false"
        >
            <f7-actions-group>
                <f7-actions-button
                    v-if="mode !== 'view'"
                    @click="updateGeoLocation(true)"
                    >{{ $t("Update Geographic Location") }}</f7-actions-button
                >
                <f7-actions-button
                    v-if="mode !== 'view'"
                    @click="clearGeoLocation"
                    >{{ $t("Clear Geographic Location") }}</f7-actions-button
                >
            </f7-actions-group>
            <f7-actions-group v-if="mapProvider">
                <f7-actions-button
                    :class="{ disabled: !transaction.geoLocation }"
                    @click="showGeoLocationMapSheet = true"
                    >{{ $t("Show on the map") }}</f7-actions-button
                >
            </f7-actions-group>
            <f7-actions-group>
        ...
        >
            <f7-actions-group
                v-if="transaction.type === allTransactionTypes.Transfer"
            >
                <f7-actions-button @click="swapTransactionData(true, false)">{{
                    $t("Swap Account")
                }}</f7-actions-button>
                <f7-actions-button @click="swapTransactionData(false, true)">{{
                    $t("Swap Amount")
                }}</f7-actions-button>
                <f7-actions-button @click="swapTransactionData(true, true)">{{
                    $t("Swap Account and Amount")
                }}</f7-actions-button>
            </f7-actions-group>
            <f7-actions-group>
                <f7-actions-button
                    v-if="transaction.hideAmount"
                    @click="transaction.hideAmount = false"
                    >{{ $t("Show Amount") }}</f7-actions-button
                >
                <f7-actions-button
                    v-if="!transaction.hideAmount"
                    @click="transaction.hideAmount = true"
                    >{{ $t("Hide Amount") }}</f7-actions-button
                >
            </f7-actions-group>
            <f7-actions-group>
                <f7-actions-button bold close>{{
        ...
        <f7-toolbar tabbar bottom v-if="mode !== 'view'">
            <f7-link
                ...
                @click="save"
            >
                <span class="tabbar-primary-link">{{
                    $t(saveButtonTitle)
                }}</span>
            </f7-link>
        </f7-toolbar>
...
export default {
    ...
    computed: {
        ...
        sourceAmountName() {
            if (this.transaction.type === this.allTransactionTypes.Expense) {
                ...
                this.transaction.type === this.allTransactionTypes.Income
            ) {
                return "Income Amount";
            } else if (
                this.transaction.type === this.allTransactionTypes.Transfer
            ) {
        ...
        sourceAccountTitle() {
            if (
                this.transaction.type === this.allTransactionTypes.Expense ||
                this.transaction.type === this.allTransactionTypes.Income
            ) {
                return "Account";
            } else if (
        ...
        transferInAmountTitle() {
            const sourceAccount =
                this.allAccountsMap[this.transaction.sourceAccountId];
            const destinationAccount =
                this.allAccountsMap[this.transaction.destinationAccountId];
            ...
            const fromExchangeRate =
                this.exchangeRatesStore.latestExchangeRateMap[
            ...
            const amountRate = this.$locale.getAdaptiveAmountRate(
                this.userStore,
        ...
        currentTimezoneOffsetMinutes() {
            return getTimezoneOffsetMinutes(
                this.settingsStore.appSettings.timeZone
            );
        },
        defaultCurrency() {
        ...
        categorizedAccounts() {
            return this.$locale.getCategorizedAccountsWithDisplayBalance(
                this.allVisibleAccounts,
                this.showAccountBalance,
                this.defaultCurrency,
                this.settingsStore,
                this.userStore,
                this.exchangeRatesStore
            );
        },
        allCategories() {
        ...
        hasAvailableExpenseCategories() {
            if (
                !this.allCategories ||
                !this.allCategories[this.allCategoryTypes.Expense] ||
                !this.allCategories[this.allCategoryTypes.Expense].length
            ) {
                return false;
            }

            const firstAvailableCategoryId = getFirstAvailableCategoryId(
                this.allCategories[this.allCategoryTypes.Expense]
            );
        ...
        hasAvailableIncomeCategories() {
            if (
                !this.allCategories ||
                !this.allCategories[this.allCategoryTypes.Income] ||
                !this.allCategories[this.allCategoryTypes.Income].length
            ) {
                return false;
            }
        ...
        hasAvailableTransferCategories() {
            if (
                !this.allCategories ||
                !this.allCategories[this.allCategoryTypes.Transfer] ||
                !this.allCategories[this.allCategoryTypes.Transfer].length
            ) {
                return false;
            }
        ...
        transactionDisplayTimezone() {
            return `UTC${getUtcOffsetByUtcOffsetMinutes(
                this.transaction.utcOffset
            )}`;
        },
        transactionDisplayTimezoneName() {
            return getNameByKeyValue(
                this.allTimezones,
                this.transaction.timeZone,
                "name",
                "displayName"
            );
        },
        transactionTimezoneTimeDifference() {
            return this.$locale.getTimezoneDifferenceDisplayText(
                this.transaction.utcOffset
            );
        },
        sourceAmountClass() {
            ...
            classes[
                this.getFontClassByAmount(this.transaction.sourceAmount)
            ] = true;
        ...
        destinationAmountClass() {
            ...
            classes[
                this.getFontClassByAmount(this.transaction.destinationAmount)
            ] = true;
    ...
    watch: {
        "transaction.sourceAmount": function (newValue, oldValue) {
            ...
            this.transactionsStore.setTransactionSuitableDestinationAmount(
                this.transaction,
                oldValue,
                newValue
            );
        ...
        "transaction.timeZone": function (newValue) {
            for (let i = 0; i < this.allTimezones.length; i++) {
                if (this.allTimezones[i].name === newValue) {
                    this.transaction.utcOffset =
                        this.allTimezones[i].utcOffsetMinutes;
                    break;
                }
            }
        },
    ...
    created() {
        ...
        if (query.id) {
            ...
            promises.push(
                self.transactionsStore.getTransaction({
        ...
        Promise.all(promises)
            ...
            .catch((error) => {
                logger.error(
                    "failed to load essential data for editing transaction",
                    error
                );
    ...
    methods: {
        onPageAfterIn() {
            ...
            if (
                this.settingsStore.appSettings.autoGetCurrentGeoLocation &&
                this.mode === "add" &&
                !this.geoLocationStatus &&
                !this.transaction.geoLocation
            ) {
        ...
        getDisplayCurrency(value) {
            return this.$locale.formatAmountWithCurrency(
                this.settingsStore,
                this.userStore,
                value,
                false
        ...
        getSecondaryCategoryName(categoryId, allCategories) {
            return getTransactionSecondaryCategoryName(
                categoryId,
                allCategories
            );
        },

<template>
    <f7-page ptr @ptr:refresh="reload" @page:afterin="onPageAfterIn">
        ...
        <f7-popover
            class="chart-data-type-popover-menu"
                ...
                >
                    <template #after>
                        <f7-icon
                            ...
                            f7="checkmark_alt"
                            v-if="query.chartDataType === dataType.type"
                        ></f7-icon>
                    </template>
                </f7-list-item>
            </f7-list>
        ...
        <f7-card
            v-if="query.categoricalChartType === allCategoricalChartTypes.Pie"
        >
            <f7-card-header class="no-border display-block">
                <div class="statistics-chart-header full-line text-align-right">
                    ...
                    <f7-link
                        href="#"
                        popover-open=".sorting-type-popover-menu"
                        >{{ querySortingTypeName }}</f7-link
                    >
                </div>
            </f7-card-header>
            <f7-card-content
            ...
            >
                <pie-chart
                    :items="[
                        { value: 60, color: '7c7c7f' },
                        { value: 20, color: 'a5a5aa' },
                        { value: 20, color: 'c5c5c9' },
                    ]"
                    :skeleton="true"
                ...
                >
                    <text
                        class="statistics-pie-chart-total-amount-title"
                        v-if="
                            categoricalAnalysisData.items &&
                            categoricalAnalysisData.items.length
                        "
                    >
                        {{ totalAmountName }}
                    </text>
                    <text
                        class="statistics-pie-chart-total-amount-value"
                        v-if="
                            categoricalAnalysisData.items &&
                            categoricalAnalysisData.items.length
                        "
                    >
                        {{
                            getDisplayAmount(
                                categoricalAnalysisData.totalAmount,
                                defaultCurrency,
                                16
                            )
                        }}
                    ...
                    <text
                        class="statistics-pie-chart-total-no-data"
                        cy="50%"
                        v-if="
                            !categoricalAnalysisData.items ||
                            !categoricalAnalysisData.items.length
        ...
        <f7-card
            v-else-if="
                query.categoricalChartType === allCategoricalChartTypes.Bar
            "
        >
            <f7-card-header class="no-border display-block">
                <div
                    class="statistics-chart-header display-flex full-line justify-content-space-between"
                >
                    <div>
                        {{ totalAmountName }}
                    </div>
                ...
                <div class="display-flex full-line">
                    <div
                        :class="{
                            'statistics-list-item-overview-amount': true,
                            'text-color-teal':
                                query.chartDataType ===
                                    allChartDataTypes.ExpenseByAccount.type ||
                                query.chartDataType ===
                                    allChartDataTypes.ExpenseByPrimaryCategory
                                        .type ||
                                query.chartDataType ===
                                    allChartDataTypes.ExpenseBySecondaryCategory
                                        .type,
                            'text-color-red':
                                query.chartDataType ===
                                    allChartDataTypes.IncomeByAccount.type ||
                                query.chartDataType ===
                                    allChartDataTypes.IncomeByPrimaryCategory
                                        .type ||
                                query.chartDataType ===
            ...
            <f7-card-content style="margin-top: -14px" :padding="false">
                <f7-list
                    class="statistics-list-item skeleton-text"
                    v-if="loading"
                >
                    <f7-list-item
                        link="#"
                    ...
                    >
                        <template #media>
                            <div class="display-flex no-padding-horizontal">
                                <div
                                    class="display-flex align-items-center statistics-icon"
                                >
                    ...
                    >
                        <template #media>
                            <div class="display-flex no-padding-horizontal">
                                ...
                                >
                                    <ItemIcon
                                        :icon-type="queryChartDataCategory"
                                        :icon-id="item.icon"
                                        :color="item.color"
                                        v-if="item.icon"
                                    ></ItemIcon>
        ...
        <f7-popover
            class="sorting-type-popover-menu"
        ...
        >
            <f7-list dividers>
                <f7-list-item
                    ...
                    :class="{
                        'list-item-selected':
                            query.sortingType === sortingType.type,
                    }"
                ...
                >
                    <template #after>
                        <f7-icon
                            class="list-item-checked-icon"
                            f7="checkmark_alt"
                            v-if="query.sortingType === sortingType.type"
                        ></f7-icon>
                    </template>
                </f7-list-item>
            </f7-list>
        ...
        <f7-toolbar tabbar bottom class="toolbar-item-auto-size">
            <f7-link
                :class="{
                    disabled:
                        reloading ||
                        query.categoricalChartDateType ===
                            allDateRanges.All.type ||
                        query.chartDataType ===
                            allChartDataTypes.AccountTotalAssets.type ||
                        query.chartDataType ===
                            allChartDataTypes.AccountTotalLiabilities.type,
                }"
                @click="
                    shiftDateRange(
                        query.categoricalChartStartTime,
            ...
            <f7-link
                :class="{
                    'tabbar-text-with-ellipsis': true,
                    disabled:
                        reloading ||
                        query.chartDataType ===
                            allChartDataTypes.AccountTotalAssets.type ||
                ...
                popover-open=".date-popover-menu"
            >
                <span
                    :class="{
                        'tabbar-item-changed':
                            query.maxTime > 0 || query.minTime > 0,
                    }"
                    >{{ dateRangeName(query) }}</span
                >
            </f7-link>
            <f7-link
                :class="{
            ...
            <f7-link
                class="tabbar-text-with-ellipsis"
                :key="chartType.type"
                v-for="chartType in allChartTypes"
                @click="setChartType(chartType.type)"
            >
                <span
                    :class="{
                        'tabbar-item-changed':
                            query.categoricalChartType === chartType.type,
                    }"
                    >{{ chartType.displayName }}</span
                >
            </f7-link>
        </f7-toolbar>

        <f7-popover
            class="date-popover-menu"
        ...
        >
            <f7-list dividers>
                <f7-list-item
                    ...
                    :class="{
                        'list-item-selected':
                            query.categoricalChartDateType === dateRange.type,
                    }"
                    :key="dateRange.type"
                ...
                >
                    ...
                    <template #footer>
                        <div
                            v-if="
                                dateRange.type === allDateRanges.Custom.type &&
                                query.categoricalChartDateType ===
                                    allDateRanges.Custom.type &&
                                query.categoricalChartStartTime &&
                                query.categoricalChartEndTime
                            "
                        >
        ...
        >
            <f7-actions-group>
                <f7-actions-button @click="filterAccounts">{{
                    $t("Filter Accounts")
                }}</f7-actions-button>
                <f7-actions-button @click="filterCategories">{{
                    $t("Filter Transaction Categories")
                }}</f7-actions-button>
            </f7-actions-group>
            <f7-actions-group>
                <f7-actions-button @click="settings">{{
                    $t("Settings")
                }}</f7-actions-button>
            </f7-actions-group>
            <f7-actions-group>
...
export default {
    ...
    computed: {
        ...
        queryChartDataTypeName() {
            const queryChartDataTypeName = getNameByKeyValue(
                this.allChartDataTypes,
                this.query.chartDataType,
                "type",
                "name",
                "Statistics"
            );
        ...
        querySortingTypeName() {
            const querySortingTypeName = getNameByKeyValue(
                this.allSortingTypes,
                this.query.sortingType,
                "type",
                "name",
                "System Default"
            );
        ...
        queryStartTime() {
            return this.$locale.formatUnixTimeToLongDateTime(
                this.userStore,
                this.query.categoricalChartStartTime
            );
        },
        queryEndTime() {
            return this.$locale.formatUnixTimeToLongDateTime(
                this.userStore,
                this.query.categoricalChartEndTime
            );
        },
        allChartTypes() {
        ...
        allDateRangesArray() {
            return this.$locale.getAllDateRanges(
                datetimeConstants.allDateRangeScenes.Normal,
                true
            );
        },
        showAccountBalance() {
        ...
        totalAmountName() {
            if (
                this.query.chartDataType ===
                    this.allChartDataTypes.IncomeByAccount.type ||
                this.query.chartDataType ===
                    this.allChartDataTypes.IncomeByPrimaryCategory.type ||
                this.query.chartDataType ===
                    this.allChartDataTypes.IncomeBySecondaryCategory.type
            ) {
                return this.$t("Total Income");
            } else if (
                this.query.chartDataType ===
                    this.allChartDataTypes.ExpenseByAccount.type ||
                this.query.chartDataType ===
                    this.allChartDataTypes.ExpenseByPrimaryCategory.type ||
                this.query.chartDataType ===
                    this.allChartDataTypes.ExpenseBySecondaryCategory.type
            ) {
                return this.$t("Total Expense");
            } else if (
                this.query.chartDataType ===
                this.allChartDataTypes.AccountTotalAssets.type
            ) {
                return this.$t("Total Assets");
            } else if (
                this.query.chartDataType ===
                this.allChartDataTypes.AccountTotalLiabilities.type
            ) {
    ...
    methods: {
        onPageAfterIn() {
            if (
                this.statisticsStore.transactionStatisticsStateInvalid &&
                !this.loading
            ) {
                this.reload(null);
            }
        ...
        reload(done) {
            ...
            if (
                self.query.chartDataType ===
                    self.allChartDataTypes.ExpenseByAccount.type ||
                self.query.chartDataType ===
                    self.allChartDataTypes.ExpenseByPrimaryCategory.type ||
                self.query.chartDataType ===
                    self.allChartDataTypes.ExpenseBySecondaryCategory.type ||
                self.query.chartDataType ===
                    self.allChartDataTypes.IncomeByAccount.type ||
                self.query.chartDataType ===
                    self.allChartDataTypes.IncomeByPrimaryCategory.type ||
                self.query.chartDataType ===
                    self.allChartDataTypes.IncomeBySecondaryCategory.type
            ) {
                ...
                self.query.chartDataType ===
                    self.allChartDataTypes.AccountTotalAssets.type ||
                self.query.chartDataType ===
                    self.allChartDataTypes.AccountTotalLiabilities.type
            ) {
        ...
        setSortingType(sortingType) {
            if (
                sortingType < this.allSortingTypes.Amount.type ||
                sortingType > this.allSortingTypes.Name.type
            ) {
        ...
        setDateFilter(dateType) {
            if (dateType === this.allDateRanges.Custom.type) {
                // Custom
        ...
        setCustomDateFilter(startTime, endTime) {
            ...
            const chartDateType = getDateTypeByDateRange(
                startTime,
                endTime,
                this.firstDayOfWeek,
                datetimeConstants.allDateRangeScenes.Normal
        ...
        shiftDateRange(startTime, endTime, scale) {
            if (
                this.query.categoricalChartDateType ===
                this.allDateRanges.All.type
            ) {
                return;
            }

            const newDateRange = getShiftedDateRangeAndDateType(
                startTime,
                endTime,
                scale,
                this.firstDayOfWeek,
        ...
        getItemLinkUrl(item) {
            return `/transaction/list?${this.statisticsStore.getTransactionListPageParams(
                statisticsConstants.allAnalysisTypes.CategoricalAnalysis,
                item
            )}`;
        },
    },

<template>
    <v-text-field
        class="text-field-with-colored-label"
...
export default {
            ...
            rules: [
                (v) => {
                    ...
                    try {
                        ...
                        return (
                            (val >= transactionConstants.minAmount &&
                                val <= transactionConstants.maxAmount) ||
                            self.$t("Amount value exceeds limitation")
                        );
                    } catch (e) {
                        console.error(e);
                        return self.$t("Amount value is not number");
    ...
    watch: {
        modelValue: function (newValue) {
            const numericCurrentValue = stringCurrencyToNumeric(
                this.currentValue
            );

            if (newValue !== numericCurrentValue) {
                const newStringValue = numericCurrencyToString(
                    newValue,
                    false,
                    true,
                    true
    ...
    methods: {
        onKeyUpDown(e) {
            ...
            if (decimalLength > 2) {
                e.target.value = e.target.value.substring(
                    0,
                    Math.min(decimalIndex + 3, e.target.value.length - 1)
                );
            ...
            try {
                ...
                let maxLength =
                    transactionConstants.maxAmount.toString().length;

                if (val < 0) {
                    maxLength =
                        transactionConstants.minAmount.toString().length;
                }
        ...
        getDisplayCurrencyPrependAndAppendText() {
            return this.$locale.getDisplayCurrencyPrependAndAppendText(
                this.currency,
